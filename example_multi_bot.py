...
方式一：环境变量
  ...
  2. 在 .env 中配置多个机器人
  3. 运行: uv run python example_multi_bot.py

方式二：JSON 配置文件
  1. 复制 bots_config.json.example 为 bots_config.json
  2. 填写机器人配置
  3. 运行: uv run python example_multi_bot.py --config bots_config.json
"""
...
async def main():
    ...
    def on_any_connected(name: str):
        print(f"[全局] 机器人已连接: {name}")

    @manager.on("bot.disconnected")
    def on_any_disconnected(name: str, reason: str):
        print(f"[全局] 机器人已断开: {name}, 原因: {reason}")

    @manager.on("bot.error")
    def on_any_error(name: str, error: Exception):
